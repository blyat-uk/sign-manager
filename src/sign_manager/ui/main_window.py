...
from sign_manager.identity import APP_NAME, APP_SLUG, ORG_NAME
...
from sign_manager.model.groups import DerivedGroupModel
...
from sign_manager.services.mpv_service import MpvPreviewWidget
from sign_manager.services.preload import FilePreloadTask, PreloadResult, PreloadSignals
from sign_manager.services.video_service import CachedVideoService, VideoService
...
from sign_manager.ui.gallery_widget import GalleryPanel, LabelGroup, compute_label_groups, _crop_to_labels_image, _best_representative_time
from sign_manager.ui.label_toolbar import LabelToolbar
from sign_manager.ui.timeline_widget import TimelineWidget
...
class WelcomeWidget(QWidget):
    ...
    def set_recent_dirs(self, dirs: list) -> None:
        ...
        from sign_manager.ui import theme
        from sign_manager.ui.recent_dirs import migrate
...
class MainWindow(QMainWindow):
    def __init__(self, *, ffmpeg_bin: str = "ffmpeg", ffprobe_bin: str = "ffprobe") -> None:
        ...
        self._video_service: VideoService = CachedVideoService(
            FFmpegVideoService(ffmpeg_bin=ffmpeg_bin, ffprobe_bin=ffprobe_bin),
            FrameCache(max_size=perf.frame_cache_size),
        )
        ...
        _init_settings = QSettings(ORG_NAME, APP_SLUG)
        ...
        from sign_manager.ui.gallery_widget import GalleryHandle
        ...
        self._settings = QSettings(ORG_NAME, APP_SLUG)
    ...
    def _add_recent_dir(self, directory: str) -> None:
        from sign_manager.ui.recent_dirs import migrate, touch
        raw = self._settings.value("recent_dirs", type=list) or []
        items = migrate(raw)
    ...
    def _on_redetect_hardware(self) -> None:
        ...
        """
        from sign_manager.services.app_settings import TIER_LABELS
    ...
    def _on_text_edited(self, label: LabelDialogue, rich_text: str) -> None:
        ...
        from sign_manager.model.ass_file import _OVERRIDE_BLOCK_RE
    ...
    def _on_gallery_toggled(self, visible: bool) -> None:
        ...
        from sign_manager.services.app_settings import save_display
